          ...
          <button
            ...
            className={`transition-all duration-300 uppercase ease-in-out py-2 px-6 lg:px-8 xl:px-10 2xl:px-12 font-bold text-gray-900 rounded-lg p-2 ${
              selectedSiteSafety === siteSafety
                ...
                : 'bg-black bg-opacity-30 text-indigo-100'
            } hover:bg-gradient-to-r hover:from-indigo-300 hover:to-indigo-200 hover:text-black focus:outline-none`}
          >
            {siteSafety}
          </button>
        ))}
      </div>
      <div className="md:hidden mt-6 mb-10 shadow-xl shadow-indigo-200/50 rounded-xl">
        <select
          value={selectedSiteSafety}
          onChange={(e) => handleSiteSafety(e.target.value)}
          className="text-center uppercase w-full px-4 py-4 bg-black bg-opacity-30 text-indigo-100 font-bold rounded-lg focus:outline-none appearance-none transition-all duration-300 ease-in-out hover:bg-opacity-60"
        >
          {siteSafetyCategories.map((siteSafety) => (
            <option key={siteSafety} value={siteSafety} className='bg-black hover:bg-indigo-200 text-indigo-100 font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline transition-all duration-300 ease-in-out hover:bg-opacity-60 hover:text-black hover:font-bold'>
              {siteSafety}
            </option>
          ))}
        </select>
